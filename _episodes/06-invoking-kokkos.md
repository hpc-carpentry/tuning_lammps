...
free-of-cost: it may require several man-years to modify an existing code to make it
...
[Kokkos](https://github.com/kokkos) strives to be an answer to this portability issue. The
primary objective of Kokkos
is to maximize the
amount of user code that can be compiled for various devices but obtaining comparable
performance to if the code
...
The **Kokkos** package in LAMMPS is implemented to gain performance with portability. Various pair
styles, fixes and atom
styles have been updated in LAMMPS to use the data structures and macros as provided by the
Kokkos C++ library so
that when LAMMPS is built with Kokkos feature enabled for a particular hardware, it can provide
optimal performance
for that hardware when all the runtime parameters are chosen sensibly. What are the things that it
supports currently?
...
* Still in developmental stage, so more features and flexibilities are expected in future
  versions of LAMMPS.
...
1. `neigh`: When you use `full` list, all the neighbors of atom `I` are stored with
   ...
   with atom `I` or `J` but not both. For GPU, a value of `full` for `neigh` keyword is
...
5. `cuda/aware`: When we use regular MPI, multiple instances of an application is
   ...
   helps to eliminate the overhead due to this copying process. But, not all HPC systems
   ...
   But, whenever a CUDA-aware MPI is available to you, you should not need to use this.


## Invoking Kokkos through input file or through command-line?

Unlike the *USER-OMP* or the *GPU* package in the
[previous episode]({{page.root}}/05-accelerating-lammps) which supports *either* OpenMP
